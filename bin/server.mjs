...
// The Approval Inbox auto-refreshes every 3s; a live SSH tmux-has-session
// check on every single refresh would hammer the worker host for no real
// benefit. Cache each non-aboardable line's status for a short window --
// same freshness spirit as Aboardable's own 15s heartbeat staleness check.
const otherLineStatusCache = new Map();
const OTHER_LINE_STATUS_TTL_MS = 10_000;
async function cachedWorkerStatus(line) {
  const cached = otherLineStatusCache.get(line.line_id);
  if (cached && Date.now() - cached.at < OTHER_LINE_STATUS_TTL_MS) return cached.status;
  const status = await getWorkerStatus(line);
  otherLineStatusCache.set(line.line_id, { at: Date.now(), status });
  return status;
}

function workerTopologyRow(worker) {
  const stateLabel = worker.state === "ready" ? "Ready" : worker.state === "working" ? "Working" : "Unavailable";
  const stateColor = worker.state === "ready" ? "#b8bdc2" : worker.state === "working" ? "#1f8a4c" : "#b23b2c";
  const checkedAt = worker.last_heartbeat_at ? new Date(worker.last_heartbeat_at).toLocaleString("en-SG", { timeZone: "Asia/Singapore", hour: "numeric", minute: "2-digit", second: "2-digit", hour12: true }) + " SGT" : "No heartbeat";
  return `<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap"><div><small>Controller inbox</small><br><strong><span style="color:#1f8a4c">●</span> Online</strong></div><span aria-hidden="true">→</span><div><small>Product worker</small><br><strong><span style="color:${stateColor}">●</span> ${stateLabel}</strong><br><small>${worker.process || "No process"}${worker.terminal_attached ? " · terminal attached" : ""} · ${checkedAt}</small></div></div>`;
}
  ...
  if (method === "tools/call") {
    ...
    if (params?.name === "read_worker_report") {
      try {
        ...
        // A successful pull clears this line's "New report" badge in the
        // Approval Inbox for every viewer, immediately -- not just a
        // click-to-dismiss in whichever single browser happens to see it.
        try { await consoleApiFetch(`/v1/lines/${encodeURIComponent(line.line_id)}/mark-report-read`, { method: "POST", body: "{}" }); } catch {}
        return result(id, {
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "GET" && url.pathname === INBOX_BASE) {
    ...
    try {
      const { lines } = await consoleApiFetch("/v1/lines");
      allLines = lines || [];
      for (const l of allLines) lineNameById.set(l.line_id, l.display_name || l.line_id);
    } catch {}
    // Server-side "has the controller pulled a report at least this recent"
    // watermark, set by resolveLine's read_worker_report handler on every
    // successful pull. SQLite CURRENT_TIMESTAMP is UTC without a "Z" suffix
    // ("2026-08-18 10:09:50"), which Date() would otherwise parse as local
    // time -- normalize before comparing.
    function sqliteTsToMs(value) {
      if (!value) return 0;
      const iso = String(value).includes("T") ? value : `${String(value).replace(" ", "T")}Z`;
      const ms = new Date(iso).getTime();
      return Number.isFinite(ms) ? ms : 0;
    }
    const lastReportReadAtByLine = new Map(allLines.map(l => [l.line_id, sqliteTsToMs(l.last_report_read_at)]));
    ...
    for (const event of reportEvents) {
      ...
      const serverRead = at && new Date(at).getTime() <= (lastReportReadAtByLine.get("aboardable-product") || 0);
      activity.push({ at, html: `<li class="report-history" data-report-id="${reportId}" data-line-id="aboardable-product"${serverRead ? ' data-server-read="true"' : ""} tabindex="0" title="Click to mark this report read"><div class="report-history-head"><strong>Report ready</strong><span class="draft-line-badge">Aboardable</span></div><pre>${message}</pre><span class="new-badge">New</span></li>` });
    }
    // The webhook-based reportEvents above only ever fires for Aboardable
    // (it's fed by a single hardcoded push endpoint on 139.99.135.10 with
    // no line concept). Any other active line's reports only ever land in
    // the reports table via its own ingestion watcher -- surface those
    // here too, explicitly labeled, so "a report flashed" always makes it
    // unambiguous which line's controller session to go activate.
    const otherLinePanels = [];
    for (const line of allLines) {
      if (line.line_id === "aboardable-product" || line.status !== "active") continue;
      let detail;
      try { detail = await consoleApiFetch(`/v1/lines/${encodeURIComponent(line.line_id)}`); } catch { continue; }
      const label = escapeHtml((line.display_name || line.line_id).replace(/\bProduct\b/gi, "").replace(/\s+/g, " ").trim());
      const lineReadAt = lastReportReadAtByLine.get(line.line_id) || 0;
      for (const report of detail?.reports || []) {
        const reportId = `${line.line_id}:${report.run_id}`;
        const summaryText = report.summary ? escapeHtml(report.summary) : `New report detected (run ${escapeHtml(report.run_id)}, state: ${escapeHtml(report.state)}). Use read_worker_report with line_id "${escapeHtml(line.line_id)}" to view its full content.`;
        const serverRead = sqliteTsToMs(report.detected_at) <= lineReadAt;
        activity.push({ at: report.detected_at, html: `<li class="report-history" data-report-id="${escapeHtml(reportId)}" data-line-id="${escapeHtml(line.line_id)}"${serverRead ? ' data-server-read="true"' : ""} tabindex="0" title="Click to mark this report read"><div class="report-history-head"><strong>Report ready</strong><span class="draft-line-badge">${label}</span></div><pre>${summaryText}</pre><span class="new-badge">New</span></li>` });
      }
      const liveStatus = await cachedWorkerStatus(detail);
      const topologyRow = workerTopologyRow(liveStatus.worker);
      const latest = detail?.reports?.[0];
      let reportPanelHtml;
      if (latest) {
        const latestId = `${line.line_id}:${latest.run_id}`;
        const latestSummary = latest.summary ? escapeHtml(latest.summary) : `New report detected (run ${escapeHtml(latest.run_id)}, state: ${escapeHtml(latest.state)}). Use read_worker_report with line_id "${escapeHtml(line.line_id)}" to view its full content.`;
        const latestServerRead = sqliteTsToMs(latest.detected_at) <= lineReadAt;
        reportPanelHtml = `<div class="latest-report-panel" data-report-id="${escapeHtml(latestId)}" data-line-id="${escapeHtml(line.line_id)}"${latestServerRead ? ' data-server-read="true"' : ""}><div class="report-history-head"><strong>Latest report</strong><span class="draft-line-badge">${label}</span></div><pre style="margin:6px 0 0;background:transparent;padding:0">${latestSummary}</pre><span class="new-badge">New</span></div>`;
      } else {
        reportPanelHtml = `<div class="latest-report-panel" data-line-id="${escapeHtml(line.line_id)}"><div class="report-history-head"><strong>Latest report</strong><span class="draft-line-badge">${label}</span></div><pre style="margin:6px 0 0;background:transparent;padding:0">No report notification received yet.</pre></div>`;
      }
      otherLinePanels.push(`<section style="background:#fff;border:1px solid #bbb;border-radius:10px;padding:13px;margin:10px 0 14px"><strong style="display:block;margin-bottom:10px">${label} worker</strong>${topologyRow}${reportPanelHtml}</section>`);
    }
