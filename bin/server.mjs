  ...
  if (req.method === "GET" && url.pathname === INBOX_BASE) {
    ...
    for (const line of allLines) {
      ...
      if (latest) {
        ...
        otherLinePanels.push(`<section style="background:#fff;border:1px solid #bbb;border-radius:10px;padding:13px;margin:10px 0 14px"><strong style="display:block;margin-bottom:10px">${label} worker</strong><div class="latest-report-panel" data-report-id="${escapeHtml(latestId)}" data-line-id="${escapeHtml(line.line_id)}"${latestServerRead ? ' data-server-read="true"' : ""}><div class="report-history-head"><strong>Latest report</strong><span class="draft-line-badge">${label}</span></div><pre style="margin:6px 0 0;background:transparent;padding:0">${latestSummary}</pre><span class="new-badge">New</span></div></section>`);
      }
    }
    ...
    const topologyHtml = `<section style="background:#fff;border:1px solid #bbb;border-radius:10px;padding:13px;margin:10px 0 14px"><strong style="display:block;margin-bottom:10px">Aboardable worker</strong>${healthNotice}<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap"><div><small>Controller inbox</small><br><strong><span style="color:#1f8a4c">●</span> Online</strong></div><span aria-hidden="true">→</span><div><small>Product worker</small><br><strong><span style="color:${stateColor}">●</span> ${stateLabel}</strong><br><small>${workerStatus.process || "No process"}${workerStatus.attached ? " · terminal attached" : ""} · ${checkedAt}</small></div></div><div class="latest-report-panel" data-report-id="${latestReportId}" data-line-id="aboardable-product"${latestReportServerRead ? ' data-server-read="true"' : ""}><div class="report-history-head"><strong>Latest report</strong><span class="draft-line-badge">Aboardable</span></div><pre style="margin:6px 0 0;background:transparent;padding:0">${latestReport}</pre><span class="new-badge">New</span></div></section>`;
